...
from app.schemas.call import (
    AutoCallReplyCreate, AutoCallReplyUpdate, AutoCallReplyRead,
    CallWebhookEvent
)
...
def get_config(
    ...
    config = session.exec(
        select(AutoCallReply).where(AutoCallReply.user_id == current_user.id)
    ).first()
    if not config:
        return None
    return config
...
@router.post("/config", response_model=AutoCallReplyRead, status_code=status.HTTP_201_CREATED)
def create_config(
    data: AutoCallReplyCreate,
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user)
):
...
@router.put("/config", response_model=AutoCallReplyRead)
def update_config(
    data: AutoCallReplyUpdate,
    session: Session = Depends(get_session),
    current_user: User = Depends(get_current_user)
):
...
async def call_webhook(event: CallWebhookEvent, session: Session = Depends(get_session)):
    """
    Webhook receiver for call.received events.
    Flow: verify HMAC → check enabled → skip groups if not allowed → reject call → send auto-reply via outbox.
    """
    print(f"[Webhook] Received event: {event.event}, session: {event.session}")
    print(f"[Webhook] Payload: from={event.payload.from_number}, id={event.payload.id}, isGroup={event.payload.isGroup}")

    if event.event != "call.received":
        print(f"[Webhook] Ignoring event: {event.event}")
    ...
    config = session.exec(
        select(AutoCallReply).where(AutoCallReply.user_id == phone.user_id)
    ).first()
